...
const uploadTransaction = async (req, res) => {
    ...
    if (req.file?.path) {
        ...
        return res.status(statusCode_1.default.Created).send({ message: "file uploaded successfully" });
    }
    else {
        return res.status(statusCode_1.default.NotFound).send({ message: "Please upload the file" });
    }
};
